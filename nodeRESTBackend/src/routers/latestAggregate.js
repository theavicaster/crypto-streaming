...
router.get('/latestAggregate', async (req, res) => {
    ...
    try {
        ...
        const latestAggregates = await cassandraClient.execute(query, [], { prepare: true });
...
router.get('/latestAggregate/:symbol_coin', async (req, res) => {
    ...
    const latestEndTime = currentUnixTime - ( currentUnixTime % config.slideDurationSeconds);
    // Cassandra stores timestamps in milliseconds
    const latestEndTimeMilli = latestEndTime * 1000;

    try {
        const query = 'SELECT * FROM rolling_aggregates WHERE symbol_coin = ? AND end_time = ?';
        const latestAggregate = await cassandraClient.execute(query, [ symbol_coin, latestEndTimeMilli ], { prepare: true });
